...
async fn file_adversary_equal_length_collision_rolls_back() {
    let root = tempfile::tempdir().unwrap();
    let store = FileEventStore::open(root.path()).await.unwrap();
    collision_rolls_back(&store).await;
}
#[tokio::test(flavor = "multi_thread")]
async fn file_adversary_rebound_retry_preserves_receipt_and_current_binding() {
    let root = tempfile::tempdir().unwrap();
    let writer = FileEventStore::open(root.path()).await.unwrap();
    let other = FileEventStore::open(root.path()).await.unwrap();
    rebound_retry(&writer, &other).await;
}
#[tokio::test(flavor = "multi_thread")]
async fn file_adversary_cancelled_guard_preserves_existing_content_and_receipt() {
    let root = tempfile::tempdir().unwrap();
    let writer = Arc::new(FileEventStore::open(root.path()).await.unwrap());
    let observer = FileEventStore::open(root.path()).await.unwrap();
    cancelled_tentative_binding(writer, &observer).await;
}
